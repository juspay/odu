...
and `unpostedContexts[]` lists GitHub status contexts not yet confirmed
(reporting debt never blocks settle — the test verdict stays the truth).
...
same `unpostedContexts`. MCP `run` tees coordinator stdout/stderr to
