 ...
 *     (pinned by `packages/cli/src/mcp/spawnSurvival.test.ts`, so a runtime bump that
 ...
 * from the old hand-built `packages/cli/src/mcp/tools.ts`.
 */
...
import { type CancelResult, cancelRun } from "@odu/execution/coordinator/cancel";
import {
  liveRunLockPid,
  signalRunLockHolder,
  waitForRunLockFree,
} from "@odu/execution/coordinator/checkoutLock";
