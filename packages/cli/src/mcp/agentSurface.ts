 ...
 * through the `runs` bespoke tool (packages/cli/src/mcp/runsTool.ts), not this live
...
import {
  ...
  toAgentNodes,
} from "@odu/execution/common/agentNodes";

/** The agent-facing vocabulary, re-exported where its consumers already look
 *  for it. What it IS lives in `packages/execution/src/common/agentNodes.ts`: the settle core
 *  speaks the same shapes, and having it import this face to learn them is the
 *  one edge that would make the engine depend on an agent surface. */
export {
  type AgentNodes,
  AgentNodesSchema,
  type AgentNodesReader,
...
// ── Durable-log fallback (ported from the old packages/cli/src/mcp/tools.ts) ─────────────
