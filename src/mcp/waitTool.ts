...
export async function waitForSettle(opts: WaitOptions): Promise<SettleVerdict> {
  ...
  // throws on abort or ends cleanly — the projected `nodes` cell swallows the
  // abort and ends the iterable rather than rejecting, so both paths must
  // classify an aborted controller as timeout/cancel, not as a settled run.
  ...
  try {
    ...
    // the timeout/cancel (the projected cell swallows abort) — that's an abort
    // verdict, never a settled one.
