 ...
 * coordinator resolves its own toplevel from its spawn cwd, and the socket,
 * lock, ledger and logs all hang off it), so a `checkout` that is not that
 * top-level points the whole kit at a `.ci` nobody serves.
 *
 ...
 *  about the argument (see `waitInput`'s `expected_sha`). */
export const checkoutField = Schema.optionalKey(
  Schema.String.check(Schema.isMinLength(1)).annotate({
    description:
      ...
      "the directory whose `.ci` tree the run owns. `run` spawns its " +
