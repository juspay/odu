...
| `node_cancel` | Cancel one node (`ci::fmt@plat`) or a whole platform lane (`@plat`); leaves the rest of the run settling. Marks targets `cancelled` (not red). |
