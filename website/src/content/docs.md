...
Use the [unified Odu skill](https://github.com/juspay/odu/tree/master/.apm/skills/odu). It includes the workflow and a Nix launcher. An APM consumer depending on `juspay/odu` receives both; use the generated MCP configuration for your agent harness.

For manual stdio configuration:
...
Bare `mcp` starts or reuses the shared service. HTTP MCP is at `http://127.0.0.1:18440/mcp`. Disconnecting an agent stops its observation, not CI.
...
The same verbs are available through `odu surface`:

```sh
odu surface --help
odu surface run_start --input '{"checkout":"/absolute/repo","expectedSha":"COMMIT_SHA","requestId":"ci-1"}' --json
odu surface run_wait --input '{"runId":"RUN_ID","deadlineMs":30000}' --json
```

Other tools: `run_cancel`, `pipeline_read`, `venue_probe`, `venue_hold`, `venue_release`, `catalog_import`, `catalog_prune`, and `protect_apply`. Use the generated Surface help for their schemas.
