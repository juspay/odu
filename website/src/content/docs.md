...
- **A node's durable log is complete.** A run that settles on its own does not close its lanes until every node has finished streaming — a recipe's summary is the last thing to arrive and used to be the first thing lost. A lane that goes silent still owing output says so in the log itself (`[odu] log truncated: …`) rather than ending mid-line. A run that does not settle on its own — cancelled, interrupted, or self-reaped after `--linger` goes idle — closes its lanes at once, so its logs may end mid-recipe (its last line still says the log ended, so `logs -f` returns). Logs are addressed by commit, not by run, so re-running the same SHA REPLACES `.ci/<sha>/<platform>/<recipe>.log` instead of appending to it.
