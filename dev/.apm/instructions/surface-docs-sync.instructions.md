---
description: When odu s user-facing surface changes (the shared service contract, the CLI that projects it, the MCP bridge), keep the README, the website docs and the odu skill in sync in the same change
applyTo: "src/main.ts,packages/service-client/src/surface.ts,packages/service-client/src/verbs.ts,packages/run-client/src/surface.ts,packages/execution/src/common/laneSurface.ts,packages/cli/src/serviceCli.ts,packages/cli/src/serviceMcp.ts"
---
